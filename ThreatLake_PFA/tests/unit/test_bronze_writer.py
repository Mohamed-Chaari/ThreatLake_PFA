...
(tests/unit/fixtures/landing/honeydb/sample.ndjson - 8 valid lines, 1
...
def test_write_bronze_writes_valid_rows_and_quarantines_malformed(
    ...
    result = write_bronze("honeydb", spark, bronze_settings)
    assert result.written == 8  # noqa: PLR2004
    assert result.quarantined == 1

    bronze_df = spark.read.format("delta").load(bronze_path("honeydb", bronze_settings))
    assert bronze_df.count() == 8  # noqa: PLR2004

    quarantine_df = spark.read.format("delta").load(quarantine_path("honeydb", bronze_settings))
...
def test_write_bronze_moves_processed_files_out_of_landing(
    ...
    landing_dir = Path(landing_path("honeydb", bronze_settings))
    assert list(landing_dir.glob("*.ndjson")), "fixture file should exist before the run"

    write_bronze("honeydb", spark, bronze_settings)
...
def test_write_bronze_is_a_noop_on_an_empty_landing_directory(
    ...
    write_bronze("honeydb", spark, bronze_settings)  # first run consumes the fixture file
    result = write_bronze("honeydb", spark, bronze_settings)  # second run: nothing left to read
